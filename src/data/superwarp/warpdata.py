...
class WarpData(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if 'input_size' in self.ms:
            self.input_size = self.ms['input_size']
        else:
    ...
    def __getitem__(self, idx: int) -> wtypes._TT:
        ...
        if self.input_size is not None:
            if self.train:
                px = random.randrange(0, w - self.input_size + 1)
                py = random.randrange(0, h - self.input_size + 1)
            else:
                px = (w - self.input_size) // 2
                py = (h - self.input_size) // 2

            img = img[py:(py + self.input_size), px:(px + self.input_size)]
