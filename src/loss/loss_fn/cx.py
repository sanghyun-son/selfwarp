...
def flow_l2(x, y, sigma=1.0, b=1.0, eps=1e-5):
    xb, xc, xh, xw = x.size()
    yb, yc, yh, yw = y.size()

    vec_x = x.view(xb, xc, -1)              # b x c x (xh x xw)
    vec_y = y.view(yb, yc, -1)              # b x c x (yh x yw)

    sq_x = vec_x.pow(2).sum(dim=1)          # b x (xh x xw)
    sq_y = vec_y.pow(2).sum(dim=1)          # b x (yh x yw)

    # Reshaping
    vec_x = vec_x.transpose(1, 2)           # b x (xh x xw) x c
    sq_x = sq_x.view(xb, -1, 1)             # b x (xh x xw) x 1
    sq_y = sq_y.view(yb, 1, -1)             # b x 1 x (yh x yw)

    mat = torch.bmm(vec_x, vec_y)           # b x (xh x xw) x (yh x yw)
    dist = sq_x - 2 * mat + sq_y            # dist[:, i, j] = D(vx[:, i], vy[:, j])
    dist = dist.view(xb, xh * xw, yh, yw)
    #print(dist, dist.size())
    dist = functional.relu(dist, inplace=True)
    den = dist.min(dim=1, keepdim=True)[0] + eps
    dist = dist / den
    weight = torch.exp((b - dist) / sigma)

    return dist, weight

def cx_loss(x, y):
    dist, weight = flow_l2(x, y, sigma=1)
    b, xhw, yh, yw = dist.size()
    k_max = weight.view(b, xhw, yh * yw).max(dim=2)[0]
    cx_score = k_max.mean(dim=1)
    cx_score = -cx_score.log()
    cx_score = cx_score.mean()
    return cx_score


class CXLoss(nn.Module):

    def __init__(self, config='vgg34'):
        super(CXLoss, self).__init__()
        self.vgg = common.extract_vgg(config)

    def forward(self, x, y):
        if not self.training:
            return 0

        vgg_x = forward_utils.forward_p(self.vgg, x)
        with torch.no_grad():
            vgg_y = forward_utils.forward_p(self.vgg, y)

        loss = (cx_loss(vgg_x, vgg_y) + cx_loss(vgg_y, vgg_x)) / 2
        return loss
...
class CSFlow:
    ...
    def create_using_L2(I_features, T_features, sigma=float(0.5), b=float(1.0)):
        ...
        #print(cs_flow.raw_distances, cs_flow.raw_distances.size())
...
if __name__ == '__main__':
    ...
    a = torch.randn(2, 16, 2, 2)
    b = torch.randn(2, 16, 3, 3)
    print(CX_loss(a, b))
    print(cx_loss(a, b))
'''