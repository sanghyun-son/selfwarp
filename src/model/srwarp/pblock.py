...
class PartialResRecon(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        #before_body = visualization.to_hsl(x.squeeze(), q=0.98)
        #utils.save_image((before_body + 1) / 2, 'example/before_body.png', padding=1)
        x = x + self.body(x, mask)
        #before_recon = visualization.to_hsl(x.squeeze(), q=0.98)
        #utils.save_image((before_recon + 1) / 2, 'example/before_recon.png', padding=1)
        x = self.recon(x, mask)
        return x