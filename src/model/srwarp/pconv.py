...
class PartialConv(nn.Conv2d):
    ...
    def forward(
        ...
        x = x + x_pad * weight
        #vis = visualization.to_hsl(x.squeeze(), q=0.98)
        #utils.save_image((vis + 1) / 2, 'example/padded_input.png', padding=1)
        x = super().forward(x)
        #vis = visualization.to_hsl(x.squeeze(), q=0.98)
        #utils.save_image((vis + 1) / 2, 'example/padded_output.png', padding=1)
        x = x * weight_after
        #vis = visualization.to_hsl(x.squeeze(), q=0.98)
        #utils.save_image((vis + 1) / 2, 'example/padded_weight.png', padding=1)
        #print('press any key')
        #input()
        return x