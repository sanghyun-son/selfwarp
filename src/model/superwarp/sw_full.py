...
class SuperWarpF(nn.Module):
    ...
    def get_kwargs(cfg: types.SimpleNamespace) -> dict:
        kwargs = {
            ...
            'kernel_net_n_feats': cfg.kernel_net_n_feats,
    ...
    def forward(
        ...
        for pi, lambda_p in enumerate(pyramids):
            ...
            grid_orig = svf.projective_grid(ref['sizes'], ms_inverse)
            grid, yi = warp.safe_region(grid_orig, p.size(-2), p.size(-1))
            if self.kernel_estimator is not None:
                ...
                if debug:
                    k, dx, dy = k
                    if pi == 2:
                        dump['dx'] = dx.detach().clone()
                        dump['dy'] = dy.detach().clone()
                        dump['grid'] = grid.detach().clone()
                        dump['grid_orig'] = grid_orig.detach().clone()
                        dump['yi'] = yi.detach().clone()
            else:
        ...
        if self.ms_blend == 'net':
            out = self.amss(ys, mask, debug=debug, const=const, **ref)
            if debug:
                out, scale_map = out
        ...
        if debug:
            #dump['grid'] = grid.detach().clone()
            #dump['yi'] = yi.detach().clone()
            dump['sizes'] = ref['sizes']
            #scale_map = mask * scale_map - 255 * (1 - mask)
            #dump['scale_map'] = scale_map.detach().clone()
            dump['x'] = x.detach().clone()
            dump['out'] = out.detach().clone()
            return out_full, dump
        else:
            mask_full = (out_full != self.fill_value).float()
