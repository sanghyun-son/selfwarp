...
class SRTrainer(_parent_class):
    ...
    def __init__(
        ...
        self.ref = ref
        self.ref.load_state_dict(self.model.state_dict())
    ...
    def get_kwargs(cfg: types.SimpleNamespace) -> dict:
        ...
        kwargs['ref'] = mloader.get_model(cfg)
        return kwargs
    ...
    def forward(
        ...
        if self.training:
            ...
            lr = samples['lr']
            if self.quads:
                ref = futils.quad_forward(self.ref, lr)
                sr = futils.quad_forward(self.model, lr)
            elif self.x8:
                ref = futils.x8_forward(self.ref, lr)
                sr = futils.x8_forward(self.model, lr)
            else:
                ref = pforward(self.ref, lr)
