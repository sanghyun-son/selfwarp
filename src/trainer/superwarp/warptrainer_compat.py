...
class ParallelWorker(nn.Module):
    ...
    def get_input(
        ...
        m = m.cpu()
        m, sizes, _ = transform.compensate_matrix(y, m)
        m_inverse = transform.inverse_3x3(m)
        ...
        '''
        x, iy, ix = crop.valid_crop(
            x, ignore_value, patch_max=1024, stochastic=False,
        )
        m_back = transform.compensate_offset(m_inverse, ix, iy)
        return x, m_back
